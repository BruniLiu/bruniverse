export const siteNavItems = [
  { label: "Home", href: "./index.html" },
  { label: "SDG 17 Goals", href: "./sdg-goals.html" },
  { label: "Act Now", href: "./act-now.html" },
  { label: "About Us", href: "./about-us.html" },
  { label: "Reference List", href: "./reference-list.html" },
];

export const sdgAgenda = {
  title: "The UN 2030 Agenda",
  summary:
    "The Sustainable Development Goals are a shared 2030 framework for ending poverty, protecting the planet, reducing inequality, and building more peaceful, resilient societies.",
  whyGuidanceMatters:
    "The world needs the SDGs because the largest challenges do not stay inside one sector. Education inequality affects health and employment. Climate risk affects cities, food systems, and public safety. Weak institutions make every other problem harder to solve. The goals give governments, communities, universities, businesses, and individuals a common language for setting priorities, measuring progress, and turning broad ethical responsibility into coordinated action.",
};

export const allSdgGoals = [
  { number: "01", title: "No Poverty", summary: "End poverty by improving basic security, opportunity, and social protection." },
  { number: "02", title: "Zero Hunger", summary: "End hunger through nutrition, food security, and sustainable agriculture." },
  { number: "03", title: "Good Health and Well-being", summary: "Improve health, prevention, care access, and well-being across all ages." },
  { number: "04", title: "Quality Education", summary: "Expand inclusive, equitable learning and lifelong skills for every community." },
  { number: "05", title: "Gender Equality", summary: "Protect equal rights, safety, participation, and opportunity for women and girls." },
  { number: "06", title: "Clean Water and Sanitation", summary: "Secure safe water, sanitation, hygiene, and sustainable water systems." },
  { number: "07", title: "Affordable and Clean Energy", summary: "Make reliable, modern, and cleaner energy accessible to everyone." },
  { number: "08", title: "Decent Work and Economic Growth", summary: "Promote productive work, fair livelihoods, and inclusive economic progress." },
  { number: "09", title: "Industry, Innovation and Infrastructure", summary: "Build resilient infrastructure and support sustainable innovation." },
  { number: "10", title: "Reduced Inequalities", summary: "Reduce unfair gaps within and among countries through inclusion and access." },
  { number: "11", title: "Sustainable Cities and Communities", summary: "Make urban and community life safer, more inclusive, and more resilient." },
  { number: "12", title: "Responsible Consumption and Production", summary: "Reduce waste and reshape how societies produce, buy, and reuse resources." },
  { number: "13", title: "Climate Action", summary: "Strengthen mitigation, adaptation, climate literacy, and disaster resilience." },
  { number: "14", title: "Life Below Water", summary: "Protect oceans, marine ecosystems, and the communities that depend on them." },
  { number: "15", title: "Life on Land", summary: "Protect forests, biodiversity, land resources, and ecosystem health." },
  { number: "16", title: "Peace, Justice and Strong Institutions", summary: "Build safety, justice, public trust, and accountable institutions." },
  { number: "17", title: "Partnerships for the Goals", summary: "Strengthen cooperation, finance, knowledge sharing, and implementation capacity." },
];

export const tenActNowActions = [
  "Save energy at home",
  "Walk, bike, or take public transport",
  "Eat more vegetables",
  "Consider your travel",
  "Throw away less food",
  "Reduce, reuse, repair, recycle",
  "Change your home's source of energy",
  "Switch to an electric vehicle",
  "Choose eco-friendly products",
  "Speak up",
];

export const teamMembers = [
  {
    name: "Liu Xiangyi",
    englishName: "Bruni",
    initials: "LX",
    photoPath: "/images/team/Bruni.jpg",
    role: "Group leader and interdisciplinary research contributor",
    background:
      "My name is Liu Xiangyi, and my English name is Bruni. I am currently studying Accounting at the Asia-Australia Business College of Liaoning University. I am also a visiting student at Shenzhen University of Advanced Technology (SUAT), where I am developing my research interests in artificial intelligence, computer vision, and computational 3D reconstruction. Alongside my accounting studies, I have been strengthening my foundation in mathematics, programming, machine learning, and scientific research.",
    aspiration:
      "In the future, I hope to become a scientist. I plan to pursue advanced study and research in artificial intelligence, computer vision, and computational 3D reconstruction. My long-term goal is to develop computational methods that help people better understand, reconstruct, and interact with the physical world through curiosity, technical ability, and persistent exploration.",
    selectedSdgs: ["SDG 16 Peace, Justice and Strong Institutions"],
    selectedActions: ["Consider your travel"],
    contributions: [
      "I coordinate the website direction and connect SDG research, visual design, and presentation structure.",
      "I bring an interdisciplinary perspective that links accounting with artificial intelligence, computer vision, and scientific research.",
      "I developed the SDG 16 research focus on digital justice and connected my Act Now topic to more careful travel choices.",
    ],
    photoStatus:
      "Profile photo confirmed.",
  },
  {
    name: "Hou Yuheng",
    englishName: "Janet",
    initials: "JH",
    photoPath: "/images/team/Janet.jpg",
    role: "SDG researcher and action evidence contributor",
    background:
      "My name is Hou Yuheng, and my English name is Janet. I am currently studying Accounting at the Asia-Australia Business College of Liaoning University.",
    aspiration:
      "I plan to complete a one-year internship at KPMG or PwC and obtain the Certified Public Accountant qualification. After graduation, I hope to work in the finance department of a state-owned enterprise or multinational corporation as a finance manager or accounting specialist.",
    selectedSdgs: ["SDG 13 Climate Action"],
    selectedActions: ["Walk, bike, or take public transport"],
    contributions: [
      "I developed the SDG 13 climate action research narrative and adverse-impact analysis for China.",
      "I organized the Act Now transport action and connected daily mobility choices with measurable emission reduction.",
      "I contributed to page structure, visual presentation, and reference-list organization.",
    ],
    photoStatus:
      "Profile photo confirmed.",
  },
  {
    name: "Han Yutong",
    englishName: "Irene",
    initials: "HY",
    photoPath: "/images/team/Irene.jpg",
    role: "Accounting student and finance pathway contributor",
    background:
      "My name is Han Yutong, and my English name is Irene. I am currently studying Accounting at the Asia-Australia Business College of Liaoning University.",
    aspiration:
      "I plan to complete a one-year internship at KPMG or PwC and obtain the Certified Public Accountant qualification. After graduation, I hope to work in the finance department of a state-owned enterprise or multinational corporation as a finance manager or accounting specialist.",
    selectedSdgs: ["SDG 2 Zero Hunger"],
    selectedActions: ["Eat more vegetables"],
    contributions: [
      "I provided profile information and a finance-oriented career pathway for the About Us page.",
      "I add an accounting and corporate-finance perspective to the group's sustainability website and presentation.",
      "I contributed the SDG 2 food security focus and the Act Now topic on eating more vegetables.",
    ],
    photoStatus:
      "Profile photo confirmed.",
  },
  {
    name: "Zheng Xinyao",
    englishName: "Elvira",
    initials: "ZX",
    photoPath: "/images/team/Elvira.jpg",
    role: "Accounting student and corporate finance pathway contributor",
    background:
      "My name is Zheng Xinyao, and my English name is Elvira. I come from Fuzhou, Fujian Province. I am currently majoring in Accounting through Liaoning University and Victoria University. Beyond my academic studies, I enjoy singing and dancing, and I actively participate in English activities organized by the university.",
    aspiration:
      "Through my studies in accounting and finance, I have built a strong foundation in financial statement analysis, risk assessment, and cost management. In the future, I aim to become a professional in corporate financial management, applying financial knowledge to support responsible business decisions and long-term organizational development.",
    selectedSdgs: ["SDG 4 Quality Education"],
    selectedActions: ["Reduce, reuse, repair, recycle"],
    contributions: [
      "I provide a corporate-finance perspective shaped by accounting, financial analysis, risk assessment, and cost management.",
      "I add communication and participation experience from university English activities to the group's collaborative presentation work.",
      "I contributed the SDG 4 education equity focus and the Act Now topic on reduce, reuse, repair, and recycle.",
    ],
    photoStatus:
      "Profile photo confirmed.",
  },
];

export const apaReferences = [
  "Britannica Editors. (2026, May 5). Carbon dioxide. Britannica. https://www.britannica.com/science/carbon-dioxide",
  "FAO, IFAD, UNICEF, WFP, & WHO. (2025). The state of food security and nutrition in the world 2025: Addressing high food price inflation for food security and nutrition. FAO. https://openknowledge.fao.org/handle/20.500.14283/cd6008en",
  "FasterCapital. (2025, April 9). Bike environmental impact: Biking vs. public transportation: Which is better for the planet. https://fastercapital.com/content/Bike-Environmental-Impact--Biking-vs--Public-Transportation--Which-Is-Better-for-the-Planet.html",
  "Feng, Y., Marek, C., & Tosun, J. (2022). Fighting food waste by law: Making sense of the Chinese approach. Journal of Consumer Policy, 45, 457-479. https://doi.org/10.1007/s10603-022-09519-2",
  "Guo, C., & Wan, B. (2022). The digital divide in online learning in China during the COVID-19 pandemic. Technology in Society, 71, Article 102122. https://doi.org/10.1016/j.techsoc.2022.102122",
  "Hanna Lazar. (n.d.). People at a railway station with luggage [Photograph]. Unsplash. https://unsplash.com/photos/SO5JaABqo2w",
  "Hodges, M. (2025, October 13). Eco-friendly commutes: Walking and biking's impact on our planet. SHUN WASTE. https://shunwaste.com/article/how-does-walking-or-riding-a-bike-affect-the-environment",
  "Huang, X. (2024). Research on the formation and evolution mechanism of the urban-rural education gap in China. Academic Journal of Humanities & Social Sciences, 7(7), 152-157. https://doi.org/10.25236/AJHSS.2024.070723",
  "Intergovernmental Panel on Climate Change. (2023). Climate change 2023: Synthesis report. https://www.ipcc.ch/report/ar6/syr/",
  "International Energy Agency. (2025). Global energy review 2025: Key findings. https://www.iea.org/reports/global-energy-review-2025/key-findings",
  "International Energy Agency. (n.d.). China: Countries and regions. https://www.iea.org/countries/china",
  "International Fund for Agricultural Development. (2020, September 29). Fighting food waste in China: Local efforts, global effects. https://www.ifad.org/en/w/opinions/fighting-food-waste-in-china-local-efforts-global-effects",
  "Jana Ohajdova. (n.d.). Green vegetables in a bowl [Photograph]. Unsplash. https://unsplash.com/photos/9nzQserYaN8",
  "Library of Congress. (2023, January 31). China: New law takes effect to combat telecom and online fraud. https://www.loc.gov/item/global-legal-monitor/2023-01-31/china-new-law-takes-effect-to-combat-telecom-and-online-fraud/",
  "Ministry of Justice of China. (2023, December 15). Anti-Telecom and Online Fraud Law of the People's Republic of China. https://en.moj.gov.cn/2023-12/15/c_948363.htm",
  "Ministry of Public Security of the People's Republic of China. (2024, May 31). Major success in combating telecom and online fraud. https://www.mps.gov.cn/n2255079/n6865805/n7355748/n7355823/c9594173/content.html",
  "Ministry of Education of the People's Republic of China. (2022). Statistical report on China's educational achievements in 2021. https://en.moe.gov.cn/documents/reports/202209/t20220924_664436.html",
  "Ministry of Education of the People's Republic of China. (2023). Statistical report on China's educational achievements in 2022. https://en.moe.gov.cn/documents/reports/202304/t20230403_1054100.html",
  "National Bureau of Statistics of China. (2021, December 31). Final statistical monitoring report on the implementation of China National Program for Child Development (2011-2020). https://www.stats.gov.cn/english/PressRelease/202112/t20211231_1825803.html",
  "National Bureau of Statistics of China. (2024, February 28). Statistical communique of the People's Republic of China on the 2023 national economic and social development. https://www.stats.gov.cn/english/PressRelease/202402/t20240228_1947918.html",
  "National Bureau of Statistics of China. (2025, February 28). Statistical communique of the People's Republic of China on the 2024 national economic and social development. https://www.stats.gov.cn/english/PressRelease/202502/t20250228_1958822.html",
  "National Oceanic and Atmospheric Administration. (n.d.). Heat Wave [Illustration]. Wikimedia Commons. https://commons.wikimedia.org/wiki/File:Heat_Wave.jpg",
  "Planet Labs. (n.d.). Chinese Solar Farm, Golmud, China [Satellite image]. Wikimedia Commons. https://commons.wikimedia.org/wiki/File:Chinese_Solar_Farm,_Golmud,_China_by_Planet_Labs.jpg",
  "Poore, J., & Nemecek, T. (2018). Reducing food's environmental impacts through producers and consumers. Science, 360(6392), 987-992. https://doi.org/10.1126/science.aaq0216",
  "Scells, H., Zhuang, S., & Zuccon, G. (2022). Reduce, reuse, recycle: Green information retrieval research. Proceedings of the 45th ACM SIGIR Conference. https://doi.org/10.1145/3477495.3531738",
  "Standing Committee of the National People's Congress. (2021). Law of the People's Republic of China on food waste. https://en.npc.gov.cn.cdurl.cn/2021-04/29/c_689496.htm",
  "Supreme People's Procuratorate of the People's Republic of China. (2025, March 8). China intensifies crackdown on telecom fraud crimes in 2024. https://en.spp.gov.cn/2025-03/08/c_1076764.htm",
  "Supreme People's Procuratorate of the People's Republic of China. (2025, March 8). Courts handle more telecom, fraud cases. https://en.spp.gov.cn/2025-03/08/c_1076747.htm",
  "United Nations. (2015, September 25). Transforming our world: The 2030 Agenda for Sustainable Development. https://sdgs.un.org/2030agenda",
  "United Nations. (n.d.). Actions for a healthy planet. https://www.un.org/en/actnow/ten-actions",
  "United Nations. (n.d.). Communications materials: Sustainable Development Goals. https://www.un.org/sustainabledevelopment/news/communications-material/",
  "United Nations. (n.d.). The 17 Goals. https://sdgs.un.org/goals",
  "United Nations Department of Economic and Social Affairs. (n.d.). Goal 2: End hunger, achieve food security and improved nutrition and promote sustainable agriculture. https://sdgs.un.org/goals/goal2",
  "United Nations Department of Economic and Social Affairs. (n.d.). Goal 4: Ensure inclusive and equitable quality education and promote lifelong learning opportunities for all. https://sdgs.un.org/goals/goal4",
  "United Nations Department of Economic and Social Affairs. (n.d.). Goal 13: Take urgent action to combat climate change and its impacts. https://sdgs.un.org/goals/goal13",
  "United Nations Department of Economic and Social Affairs. (n.d.). Goal 16: Peace, justice and strong institutions. https://sdgs.un.org/goals/goal16",
  "United Nations Development Programme. (n.d.). A circular economy aligns environmental protection with long-term economic and social well-being. https://www.undp.org/chemicals-waste/our-work/circular-economy",
  "UNESCO. (2026). China: 2026 GEM report country case study. https://www.unesco.org/gem-report/en/2026-gem-report-country-case-studies/china",
  "United Nations Environment Programme. (2024). Food Waste Index Report 2024. https://www.unep.org/resources/publication/food-waste-index-report-2024",
  "UNICEF China. (2024). Children in China: An atlas of social indicators 2024. https://www.unicef.cn/en/media/28801/file/Children%20in%20China%3A%20An%20Atlas%20of%20Social%20Indicators%202024.pdf",
  "UNICEF China. (n.d.). Education. https://www.unicef.cn/en/what-we-do/education",
  "UN Climate Change & Doconomy. (n.d.). Lifestyle Calculator by Doconomy and the UNFCCC Secretariat. https://lifestylecalculator.com/unfccc",
  "UN Climate Change & Doconomy. (2026). Method Lifestyle Calculator. https://lifestylecalculator.com/files/MethodLifestyleCalculator.pdf",
  "United Nations Environment Programme. (2024). Global Waste Management Outlook 2024: Beyond an age of waste. https://www.unep.org/resources/global-waste-management-outlook-2024",
  "U.S. Environmental Protection Agency. (n.d.). Urban heat island profile [Illustration]. Wikimedia Commons. https://commons.wikimedia.org/wiki/File:Urban_heat_island_(Celsius).png",
  "Karola G. (n.d.). Figurine of the Lady Justice on a table [Photograph]. Pexels. https://www.pexels.com/photo/figurine-of-the-lady-justice-on-a-table-7876148/",
  "Mikhail Nilov. (n.d.). Shocked man staring wide-eyed on a computer [Photograph]. Pexels. https://www.pexels.com/photo/shocked-man-staring-wide-eyed-on-a-computer-6964367/",
  "Sora Shimazaki. (n.d.). Professional male judge working on laptop with gavel [Photograph]. Pexels. https://www.pexels.com/photo/professional-male-judge-working-on-laptop-with-gavel-5668772/",
  "Tima Miroshnichenko. (n.d.). A person with handcuffs holding a sign that says fraud [Photograph]. Pexels. https://www.pexels.com/photo/a-person-with-handcuffs-holding-a-sign-that-says-fraud-6266506/",
  "Tima Miroshnichenko. (n.d.). Close-up shot of system hacking [Photograph]. Pexels. https://www.pexels.com/photo/close-up-shot-of-system-hacking-5380792/",
  "Tima Miroshnichenko. (n.d.). Man in black sweater holding money [Photograph]. Pexels. https://www.pexels.com/photo/man-in-black-sweater-holding-money-6266308/",
  "Wikimedia Commons contributors. (n.d.). Cybersecurity [Illustration]. https://commons.wikimedia.org/wiki/File:Cybersecurity.png",
  "Wikimedia Commons contributors. (n.d.). QR codes for mobile pay in China [Photograph]. https://commons.wikimedia.org/wiki/File:QR_codes_for_mobile_pay_in_China.jpg",
  "Wikimedia Commons contributors. (n.d.). Shuozhou coal power plant [Photograph]. https://commons.wikimedia.org/wiki/File:Shuozhou_coal_power_plant.JPG",
  "World Meteorological Organization. (2025). State of the global climate 2024. https://wmo.int/publication-series/state-of-global-climate/state-of-global-climate-2024",
  "World Bank Group. (2022). China country climate and development report. https://www.worldbank.org/en/country/china/publication/china-country-climate-and-development-report",
  "World Food Prize Foundation. (n.d.). 2004: Monty P. Jones and Yuan Longping. https://www.worldfoodprize.org/en/laureates/20002009_laureates/2004_jones_and_yuan/",
  "Zhang, S., Zhang, C., Cai, W., Bai, Y., Callaghan, M., Chang, N., Chen, B., et al. (2023). The 2023 China report of the Lancet Countdown on health and climate change. The Lancet Public Health, 8(12), e978-e995. https://doi.org/10.1016/S2468-2667(23)00245-1",
];
